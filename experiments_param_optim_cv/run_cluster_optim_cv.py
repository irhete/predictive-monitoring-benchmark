...
outfile = os.path.join(home_dir, results_dir, "cv_results_cluster_%s_%s.csv"%(dataset_ref, method_name))      
                ...
                for hmm_n_states in hmm_n_statess:
                    ...
                    for rf_max_features in rf_max_featuress:
                        ...
                        for cl in range(n_clusters):
                            ...
                            relevant_train_cases = data_freqs[cluster_assignments == cl].index
                            relevant_test_cases = test_data_freqs[test_cluster_assignments == cl].index

                            if len(relevant_test_cases) == 0:
                                continue
                            elif len(relevant_train_cases) == 0:
                                current_cluster_preds = [0.5] * len(relevant_test_cases)
                        ...
                        fout.write("%s;%s;%s;%s;%s;%s;%s;%s\n"%(part, dataset_name, method_name, rf_max_features, n_clusters, hmm_n_states, "auc", auc))
                        fout.write("%s;%s;%s;%s;%s;%s;%s;%s\n"%(part, dataset_name, method_name, rf_max_features, n_clusters, hmm_n_states, "precision", prec))
                        fout.write("%s;%s;%s;%s;%s;%s;%s;%s\n"%(part, dataset_name, method_name, rf_max_features, n_clusters, hmm_n_states, "recall", rec))
                        fout.write("%s;%s;%s;%s;%s;%s;%s;%s\n"%(part, dataset_name, method_name, rf_max_features, n_clusters, hmm_n_states, "fscore", fscore))
